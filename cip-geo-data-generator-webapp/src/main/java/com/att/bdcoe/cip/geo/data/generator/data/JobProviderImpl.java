...
public class JobProviderImpl implements JobProvider{
    ...
    public void write(Job job) throws IOException{
        ...
        if(job.getFileName()!=null?job.getFileName().trim().length()<=0:true) job.setFileName(Job.defaultFileName);
        else if(!job.getFileName().endsWith(FileType.getFileExtension(FileType.Job))) job.setFileName(job.getFileName().concat(".").concat(FileType.getFileExtension(FileType.Job)));
