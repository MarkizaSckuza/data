...
//        fillOpacity: 0.35,
    ...
    launch : function() {
        ...
        var fileStore = Ext.create('DataGenerator.store.FilesStore');
    ...
    onOptionsLoaded: function(response){
        var app = DataGenerator.getApplication();
    ...
    onFilesLoaded: function(response) {
        var app = DataGenerator.getApplication();
        app.files = Ext.JSON.decode(response.responseText);
...
debugger;
    ...
    calculateBoundsAndCenter: function(options) {
        ...
        options.routesCenter = new google.maps.LatLng(lat, lng)
    },
    ...
    updateOptionsObject : function() {
        ...
        Ext.data.StoreManager.lookup('wifiZoneStore').each(function(rec)
            {
                app.options.wifi.push({id: index,
                    lat:rec.data.lat,
                    lng: rec.data.lng,
